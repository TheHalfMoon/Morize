...
- `rust-fuzz/libfuzzer` / `libfuzzer-sys 0.4.13` at release merge `719e4efb9b8857ebaa782ae59376c8cbb78fed0f` — **DEV_DEPENDENCY / FUZZ_ONLY**. Upstream declares `(MIT OR Apache-2.0) AND NCSA`; the bundled libFuzzer sources are NCSA-licensed and the wrapper is MIT/Apache-2.0. Admission therefore requires fuzz-only isolation, exact lockfile closure, preserved license/notice documentation, and no linkage into Morize runtime artifacts. Upstream documentation states sanitizer-backed libFuzzer execution is Linux-only; Morize must not make Windows product CI depend on this harness.
