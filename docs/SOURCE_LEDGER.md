...
- `convaiinnovations/laya` — Hugging Face model artifact observed 2026-09-20; Apache-2.0; 421.3M-parameter typed decision model. Current model artifact SHA-256: `891102d372688fc2a094dac56a384bc537b87c63f21f9f3dac0be2b7cbc8d86c`. Candidate value: bounded `choice`/`score`/`noul` decisions, calibrated probabilities, explicit confidence/escalation behavior, and non-generative output. **REFERENCE / DEPEND_OPTIONAL** for P8 qualification only; exact model/runtime/configuration identity, local resource cost, calibration, abstention, license/notice, and safety evidence are required before admission. Laya output is evidence, never Morize authority.
